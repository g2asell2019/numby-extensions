...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($2, mangaId) => {
    ...
    const titles = [];
    titles.push(decode($2("#top > section.flex > section > h1").first().text()?.trim() ?? ""));
    ...
    const description = decode($2("section:nth-child(3) > ul > li > p").text().trim() ?? "");
    const arrayTags = [];
  ...
  var parseChapters = async ($2, mangaId) => {
    ...
    for (const chapter of $2("body > div > a > span.grow.flex.items-center.gap-2 > span:nth-child(1)").toArray()) {
      ...
      const title = decode($2(chapter).text().trim());
  ...
  var parseHomeSections = ($2, sectionCallback) => {
    ...
    for (const manga of $2("section:nth-child(3) > div > a").toArray()) {
