...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($2, mangaId) => {
    ...
    const titles = [];
    titles.push((0, import_html_entities.decode)($2("#top > section.flex > section > h1").first().text()?.trim() ?? ""));
    console.log(`Found title: ${titles.toString()}`);
    const image = $2("section:nth-child(3) > picture > img").attr("src") ?? "";
    const author = $2("ul > li:nth-child(1) > span").text().trim();
    const description = (0, import_html_entities.decode)($2("section:nth-child(3) > ul > li > p").text().trim() ?? "");
    const arrayTags = [];
    console.log(`Parsing tags ${$2("#top > section > section > section > ul > li:nth-child(2) > span > a").toArray().length} for mangaId: ${mangaId}`);
    for (const tag of $2("#top > section > section > section > ul > li:nth-child(2) > span > a").toArray()) {
    ...
    const rawStatus = $2("section.hidden > ul > li:nth-child(4) > a").text().trim();
    let status = "ONGOING";
  ...
  var parseChapters = async ($2, mangaId) => {
    ...
    const chapters = [];
    let sortingIndex = 0;
    console.log(` Found chapters: ${$2("body > div > a > span.grow.flex.items-center.gap-2 > span:nth-child(1)").length} `);
    for (const chapter of $2("body > div > a > span.grow.flex.items-center.gap-2 > span:nth-child(1)").toArray()) {
      const chapterElement = $2(chapter).parent().parent();
      const title = (0, import_html_entities.decode)($2(chapter).text().trim());
      const IDRegex = $2(chapter).parent().parent().attr("href")?.replace(/\/$/, "").match("/chapters/(.*?)$");
    ...
    if (chapters.length == 0) {
      throw new Error(`[DEBUG] Couldn't find any chapters for mangaId: ${mangaId}! with found chapters: ${$2("body > div > a > span.grow.flex.items-center.gap-2 > span:nth-child(1)").length}`);
    }
  ...
  var parseChapterDetails = async ($2, mangaId, chapterId) => {
    ...
    const pages = [];
    for (const img of $2("body > section > img").toArray()) {
  ...
  var parseHomeSections = ($2, sectionCallback) => {
    ...
    for (const manga of $2("body > main > section > section > article.flex").toArray()) {
      const title = $2("div:nth-child(2) > a > div.truncate", manga).text()?.trim() ?? "";
      const IDRegex = $2("div > a", manga).attr("href")?.replace(/\/$/, "").match("/series/(.*?)/");
    ...
    for (const manga of $2("body > main > section > section > section:nth-child(4) > div > a").toArray()) {
      ...
      const id = IDRegex && IDRegex[1] ? IDRegex[1] : "";
      const image = `https://temp.compsci88.com/cover/fallback/${id}.jpg`;
      if (!id || !title) continue;
    ...
    for (const manga of $2("body > main > section > section > section > article").toArray()) {
      const image = $2("img", manga).first().attr("src") ?? "";
      const title = $2("a > div:nth-child(1) > div", manga)?.text().trim() ?? "";
      const IDRegex = $2("a", manga).attr("href")?.replace(/\/$/, "").match("/series/(.*?)/");
      const id = IDRegex && IDRegex[1] ? IDRegex[1] : "";
      const subtitle = displayTime($2("time", manga).first().text().trim()) ?? "";
  ...
  var parseViewMore = ($2) => {
    ...
    const manga = [];
    const collectedIds = [];
    for (const obj of $2("body > article > section:nth-child(1)").toArray()) {
  ...
  var parseTags = ($2) => {
    ...
    const arrayTags = [];
    for (const tag of $2("body > main > div > section > section:nth-child(8) > div > div.collapse-content > fieldset > label").toArray()) {
      const title = $2("input[type=hidden]", tag).attr("value") ?? "";
      if (!title) continue;
  ...
  var parseSearch = ($2) => {
    ...
    const collectedIds = [];
    for (const obj of $2("body > article > section:nth-child(1)").toArray()) {
      let image = $2("picture > img", obj).first().attr("src") ?? "";
      const title = $2("a > article > div > div > div", obj).text()?.trim() ?? "";
      const IDRegex = $2("a", obj).attr("href")?.replace(/\/$/, "").match("/series/(.*?)/");
      const id = IDRegex && IDRegex[1] ? IDRegex[1] : "";
  ...
  var isLastPageSearch = ($2) => {
    let isLast = false;
    const pages = $2('button[hx-get*="/search/data"]').first().text() ?? "";
    isLast = pages ? false : true;
    return isLast;
  };
  var formatTagSearch = (tags) => {
    if (!tags || tags.length === 0) return "";
    return `${tags?.map((x) => "included_tag=" + x.id + "&").join("")}`;
  };
  var displayTime = (createdAt) => {
    const now = dayjs();
  ...
  var WeebCentral = class _WeebCentral {
    constructor() {
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      metadata = !isLastPageSearch($2) ? { page: page + 1, offset: offset + limit, limit } : void 0;
    ...
    async getSearchResults(query, metadata) {
      ...
      console.log(`Is last page search ${isLastPageSearch($2)}`);
      metadata = !isLastPageSearch($2) ? { page: page + 1, offset: offset + limit, limit } : void 0;
      return App.createPagedResults({
        results: manga,
        metadata
...
Please go to the homepage of <${_WeebCentral.name}> and press the cloud icon.`);
      }
    }
